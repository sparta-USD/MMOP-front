...
async function handlePerfumeInfo(){
    ...
    const response = await fetch('http://3.39.240.251/perfume/'+url_detail_perfume,{
        method: 'GET',
    })
    .then(response => {
        if(!response.ok){
        ...
        review_data.forEach(data => {
            ...
            review_list.innerHTML = `
                ...
                <div class="review_card_body">
                    ...
                    ${data['image'] ? `<div class="review_result_image_box"><img class="review_result_image" src="http://3.39.240.251${data['image']}" alt="No Image"></div>` : ""}
                </div>
            `;
...
async function handleRecommend() {
    url_detail_perfume = getParams("perfume");
    const response = await fetch('http://3.39.240.251/perfume/'+url_detail_perfume+'/recommend/', {
        headers: {
...
async function handlePerfumeLike() { // 5-1. 찜하기 버튼 클릭 시 상태변경 API 통신
    ...
    const response = await fetch('http://3.39.240.251/perfume/'+url_detail_perfume+'/like/', {
        headers: {
