...
async function searchPerfume(){
    ...
    const response = await fetch(`http://127.0.0.1:8000/perfume/?search=${search}&ordering=${ordering}&page=${page}`,{
        ...
        // 페이지네이션
        document.querySelector(".first_page a").setAttribute("href",`/search.html?search=${search}&ordering=${ordering}&page=1`);
        document.querySelector(".last_page a").setAttribute("href",`/search.html?search=${search}&ordering=${ordering}&page=${response_json['last']}`);
        document.querySelector(".next_page a").setAttribute("href",`/search.html?search=${search}&ordering=${ordering}&page=${response_json['next']?response_json['next']: response_json['last']}`);
        document.querySelector(".prev_page a").setAttribute("href",`/search.html?search=${search}&ordering=${ordering}&page=${response_json['previous']?response_json['previous']: 1}`);
        curr_page = Number(page)
        for(i=-2; i<3; i++){
            document.querySelector(`.page_${i} a`).setAttribute("href",`/search.html?search=${search}&ordering=${ordering}&page=${curr_page+i}`);
