...
async function handleCustomDetail(){
    ...
    const response = await fetch(`https://api.mmop-perfume.com/custom_perfume/${get_custom_perfume}/`, {
        headers: {
        ...
        if(response_json['logo']!=null){
            document.querySelector(".logo_image img").src = `https://api.mmop-perfume.com${result.logo}`
        }
        else{
